...
class DatabaseObject
{
    ...
    public static $page_path = "/public/admin/crud/data/";
    ...
    public static function initialize($page_path = "/public/admin/crud/data/")
    {
        if (!empty($page_path)) {
            if (is_dir($page_path)) {
                static::$page_path = $page_path;
            } else {
                die("The path is not a directory");
            }
        }

        static::$page_manage = static::$page_path . "manage_data.php?class_name=" . __CLASS__;
        static::$page_new = static::$page_path . "new_ajax.php?class_name=" . __CLASS__;
        static::$page_edit = static::$page_path . "edit_data.php?class_name=" . __CLASS__; //
        static::$page_delete = static::$page_path . "delete_ajax.php?class_name=" . __CLASS__;
    }
...
//        return ($database->affected_rows() == 1) ? true : false;
    ...
    protected function sanitized_attributes()
    {
        ...
        $clean_attributes = [];
        foreach ($attributes as $key => $value) {
            $clean_attributes[$key] = htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
        }
...
//        return $attributes;
    ...
    {
        $pdo = self::getPDO();
        $sql = "SELECT SUM({$field}) FROM " . static::$table_name . " {$where}";
        $stmt = $pdo->query($sql);
        $row = $stmt->fetch();
    ...
    public static function sum_field_where_by_sql($sql, $params = [])
    {
        $pdo = self::getPDO();
        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
...
//        static::$page_delete = "delete_{$data}.php?class_name=" . get_called_class();
    ...
    public static function find_column_name()
    {
        ...
        foreach ($fields as $f) {
            $output .= "<li class='list-group-item'>{$f}</li>";
        }
        $output .= "</ul>";

        $output .= "\$db_fields = ['" . implode("','", $fields) . "']<br><hr>";
...
//                    $output.= "<td $style class='text-right'>".number_format ( $this->$fieldname,2)."</td>";
    ...
    //new way of the ajax search of manage data
    public static function get_where_clause($search)
    {
        $WhereClause = " WHERE ";
        foreach (static::$db_field_search as $field) {
            if ($field == 'id') {
                continue;
            }
            if (in_array($field, static::$db_fields)) {
        ...
        $WhereClause = substr_replace($WhereClause, '', -3);
...
//        return $data[$field];
    ...
    public static function datatable_get_columns_config($is_position = true)
    {
        ...
        $data = [];
        if ($is_position) {
            $data[] = ["data" => 0, "title" => "+", "className" => "control", "orderable" => false];
        } else {
            $data[] = ["data" => null, "title" => "+", "className" => "control", "orderable" => false];
        }
        ...
        foreach (static::$db_fields as $field) {
            $i++;
            if ($is_position) {
                $data[] = ["data" => $i, "title" => ucwords(str_replace("_", " ", $field)), "type" => static::datatable_get_column_config_type($field)];
            } else {
                $data[] = ["data" => $field, "title" => ucwords(str_replace("_", " ", $field)), "type" => static::datatable_get_column_config_type($field)];
            }

        }
        if ($is_position) {
            $data[] = ["data" => $i + 1, "title" => "Actions", "orderable" => false];
        } else {
            $data[] = ["data" => 'actions', "title" => "Actions", "orderable" => false];
        }
    ...
    public static function datatable_thead_tr_th($is_position = true)
    {
        // $is_position = true is working only
        $is_position = true;
