...
// 'currency_id','Account','debitor','creditor'
    ...
    protected static $db_fields = ['id', 'chauffeur', 'pseudo', 'autres', 'date_course', 'heure_course', 'depart', 'arrive', 'commentaire', 'document', 'input_date',];

    protected static $required_fields = ['chauffeur', 'pseudo', 'date_course', 'heure_course', 'depart', 'arrive'];

    protected static $db_fields_table_display_short = ['id', 'chauffeur', 'pseudo', 'autres', 'date_course', 'heure_course', 'depart', 'arrive'];

    protected static $db_fields_table_display_full = ['id', 'chauffeur', 'pseudo', 'autres', 'date_course', 'heure_course',  'depart', 'arrive', 'commentaire', 'document',];
    ...
    public static $get_form_element = ['chauffeur', 'pseudo', 'autres', 'date_course', 'heure_course', 'depart', 'arrive', 'commentaire', 'document',];
...
//        echo $diff->format("%R%a days");
    ...
    public static function sqlresult($courses, $title = "", $is_mail = false)
    {
        ...
        foreach ($courses as $course) {
            ...
            if (User::is_admin()) {
                $edit = "<a href='/public/admin/crud/ajax/edit_ajax.php?class_name=Course&id=$course->id'>$course->pseudo</a>";
            ...
            $mydate = "$course->date_course $course->heure_course";
            ...
            $msg .= "<h3 lang='fr'>$coming_up $edit   : <b>" . h($course->pseudo) . "</b> $myday</h3>";
            $mail .= "<p  lang='fr'>$coming_up_mail $edit: <b>" . h($course->pseudo) . "</b>";

            $date = new DateTimeImmutable($course->date_course);

            $mydate = "$course->start_date $course->heure_course";
...
//        $msg.=$hour_add_1."<br>";
        ...
        $courses = Course::find_by_sql("SELECT * FROM course WHERE date_course ='$dt'  ORDER BY date_course,heure_course ");
        ...
        $courses = Course::find_by_sql("SELECT * FROM course WHERE date_course = '$tomorrow' ORDER BY date_course,heure_course   ");
        ...
        $courses = Course::find_by_sql("SELECT * FROM course WHERE datetime_course >= '$dtTime' ORDER BY date_course,heure_course   ");
...
//        }
        ...
        $this->datetime_course = "$this->date_course $this->heure_course";
        if (!$this->input_date) {
